...
from rumex.runner import IgnoredStep, MatchingFunctionNotFoundError
...
def test_success():
    ...
    @steps(r"we have 2")
    def then_(*, context):
        assert context.value == 2  # noqa: PLR2004
...
def test_parameterized_step_with_type_annotations():
    ...
    def given_(num_a: TimesTwo, num_b: float):
        assert num_a.value == 4  # noqa: PLR2004
        assert num_b == 13.5  # noqa: PLR2004
...
def test_parameterized_step_with_some_annotations():
    ...
    def given_(a: int, b, c: float, d):
        assert a == 1
        assert b == "2"
        assert c == 3.0  # noqa: PLR2004
        assert d == "4"
...
def test_scenario_with_a_description_and_a_step():
    text = textwrap.dedent("""
...
def test_unimplemented_scenario():
    text = textwrap.dedent("""
        Name: Test file.
    ...
    assert isinstance(failed_1.exception, MatchingFunctionNotFoundError)
    assert not ignored.success
    assert isinstance(ignored, IgnoredStep)
    assert not failed_2.success
    assert isinstance(failed_2.exception, MatchingFunctionNotFoundError)
